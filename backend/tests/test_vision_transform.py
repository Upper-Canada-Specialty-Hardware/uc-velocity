...
def test_workorder_force_closed_predicate():
    ...
    assert t.workorder_force_closed({"blnForceClosed": True}) is True
    assert t.workorder_force_closed({"chrStatus": "Force Closed by DPowell."}) is True
    assert t.workorder_force_closed({"chrStatus": "FORCE CLOSED"}) is True
    # A normally-closed or open workorder is NOT force-closed.
    assert t.workorder_force_closed({"chrStatus": "Closed"}) is False
    assert t.workorder_force_closed({"chrStatus": "Open"}) is False
    assert t.workorder_force_closed({}) is False


def test_force_closed_overrides_partial_shipment():
    # A partially-shipped line normally derives as partly pending...
    partial = {"intWorkorderID": 1, "intProductName": 2, "intQuantity": 10,
               "curUnitPrice": "5", "curNetPrice": "4", "intTotalShippedQuantity": 4}
    normal = t.transform_workorder_line(partial, "part")
    assert (normal["qty_fulfilled"], normal["qty_pending"]) == (4, 6)
    # ...but when the owning workorder is force-closed, the line is fully fulfilled.
    forced = t.transform_workorder_line(partial, "part", force_closed=True)
    assert (forced["qty_fulfilled"], forced["qty_pending"]) == (10, 0)
    # Close-state only: ordered quantity and pricing are untouched by the override.
    assert forced["quantity"] == 10
    assert forced["unit_price"] == normal["unit_price"]
    assert forced["base_cost"] == normal["base_cost"]


def test_transform_staff_reads_real_field_names():
    # Uses the REAL tblEmployees field names (chrFirstName / blnLead / ...), not the
    # xlsx datasheet captions. bln* yes/no serialize as "1"/"0" in the export -> to_bool.
    # (Validated end-to-end against the real tblEmployees.xml; inline fixtures here keep
    # the test hermetic -- no repo-external file, so it runs in CI.)
    bill = t.transform_staff({
        "EmployeeID": 1, "chrFirstName": "Bill", "chrLastName": "Budden",
        "chrAccountName": "BBudden", "blnLead": "0", "blnInstaller": "1", "blnManager": "0",
        "chrAddress": "1214 Victoria Park Ave.", "chrCity": "Toronto", "chrPostalCode": "M3K 1J7",
        "chrHomePhone": "(416) 750-0556", "chrMobile": "(416) 268-2954", "chrEmailAddress": "",
    })
    assert bill["staff_legacy_id"] == 1 and bill["name"] == "Bill Budden"
    assert bill["address"] == "1214 Victoria Park Ave." and bill["postal_code"] == "M3K 1J7"
    assert bill["staff_roles"] == "Installer"
    c = bill["contacts"][0]
    assert c["phone"] == "(416) 750-0556" and c["cell"] == "(416) 268-2954" and c["email"] is None

    # Multiple roles land comma-joined in a stable order.
    dave = t.transform_staff({"EmployeeID": 36, "chrFirstName": "Dave", "chrLastName": "Powell",
                              "blnLead": "1", "blnInstaller": "0", "blnManager": "1"})
    assert dave["staff_roles"] == "Lead, Manager"

    # Sparse row: no address/postal/contact fields -> None; a PERMISSION flag is not a role.
    basil = t.transform_staff({"EmployeeID": 47, "chrFirstName": "Basil", "chrLastName": "Marcus",
                               "blnLead": "1", "blnInstaller": "0", "blnManager": "0",
                               "blnSeeYtoDateTotals": "1"})  # permission, not a role -> ignored
    assert basil["address"] is None and basil["postal_code"] is None
    assert basil["staff_roles"] == "Lead" and basil["pst"] is None and basil["type"] == "staff"

    # Same name, different EmployeeID -> two DISTINCT staff (the two Cory DaSilva).
    corys = t.extract_staff([
        {"EmployeeID": 72, "chrFirstName": "Cory", "chrLastName": "DaSilva"},
        {"EmployeeID": 73, "chrFirstName": "Cory", "chrLastName": "DaSilva"},
    ])
    assert [s["staff_legacy_id"] for s in corys] == [72, 73]
    assert corys[0]["staff_roles"] is None  # no role flags -> None


def test_po_header_vendor_po_number_and_expected_delivery():
    # vendor_po_number <- chrPONumber (text); expected_delivery_date best-effort parses
    # the free-text chrDateRequired -- a real date string becomes a date.
    po = t.transform_po({"PurchaseOrderID": 79, "intProjectID": 1833, "intVendorID": 52,
                         "chrPONumber": "A1853-00001-PP", "chrDateRequired": "2019-06-15"})
    assert po["vendor_po_number"] == "A1853-00001-PP"
    assert po["expected_delivery_date"] == datetime(2019, 6, 15)
    # Free text that is NOT a date -> None (no garbage date); empty PO number -> None.
    asap = t.transform_po({"PurchaseOrderID": 80, "chrPONumber": "", "chrDateRequired": "ASAP"})
    assert asap["expected_delivery_date"] is None
    assert asap["vendor_po_number"] is None


# ---------------------------------------------------------------------------
# transform_invoice / transform_invoice_line (reconstructed from ...Shipped)
# ---------------------------------------------------------------------------

def test_transform_invoice_keys_by_number_and_uses_earliest_ship_date():
    # An invoice's legacy_id IS its (globally-unique) Vision invoice number; the invoice
    # date is the EARLIEST ship date in the group; Vision's full number is preserved.
    events = [
        {"dtmShipDate": datetime(2007, 5, 7), "chrFullInvoiceNumber": "920-1583-2"},
        {"dtmShipDate": datetime(2007, 5, 3), "chrFullInvoiceNumber": "920-1583-2"},
    ]
    inv = t.transform_invoice(1583, 920, events)
    assert inv["invoice_legacy_id"] == 920           # -> stable legacy key
    assert inv["workorder_legacy_id"] == 1583        # -> parent quote
    assert inv["invoice_date"] == datetime(2007, 5, 3)   # earliest shipment in the group
    assert inv["full_invoice_number"] == "920-1583-2"    # kept for the invoice notes


def test_transform_invoice_line_maps_event_keys_and_billed_quantity():
    # A shipment event -> invoice line: its own PK (WorkorderPartID), the parent invoice
    # number, the quote LINE key (intWorkorderPartID), unit price and quantity BILLED.
    row = {"WorkorderPartID": 46, "intInvoiceNumber": 920, "intWorkorderPartID": 132,
           "intProductID": 81, "curUnitPrice": 150.3, "intShipQuantity": 2, "intQuantity": 5}
    line = t.transform_invoice_line(row, "part")
    assert line["event_legacy_id"] == 46             # this shipment event's PK
    assert line["invoice_legacy_id"] == 920          # -> parent invoice
    assert line["line_legacy_id"] == 132             # -> the migrated quote line
    assert line["item_type"] == "part"
    assert line["product_legacy_id"] == 81
    assert line["unit_price"] == 150.3
    assert line["ship_qty"] == 2                      # quantity billed on THIS invoice
    assert line["ordered_qty"] == 5
